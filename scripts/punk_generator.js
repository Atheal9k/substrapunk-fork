...
const partTypes = [
  ...
  },
  {
    ...
    count: 5,
    offset: { x: -560, y: 0 },
    attrNames: ["", "Asian Eyes", "Sun Glasses", "Red Glasses", "Round Eyes"],
    attrSex: ["u", "u", "u", "u", "u"],
    required: true,
  },
  {
    ...
    count: 4,
    offset: { x: -560, y: 0 },
    attrNames: ["", "Left Earring", "Right Earring", "Two Earrings"],
    attrSex: ["u", "u", "u", "u"],
    required: true,
  },
  {
  ...
  },
  {
    ...
    attrSex: ["m", "u", "u", "u", "u", "m", "u", "m", "m", "f", "f", "f"],
    required: true,
  },
  {
    name: "access/acc",
...
function checkAttributeCompatibility(codeArr) {
  ...
  for (let i = 0; i < partTypes.length; i++) {
    if (partTypes[i].name == "hair/hair" && codeArr[i] == 10) blondeHair = true;
    if (partTypes[i].name == "ears/ears" && codeArr[i] >= 2) earring = true;
  }
...
async function generateFaces() {
  ...
  let codeArr = [];
  for (let i = 0; i < partTypes.length; i++) {
    if (partTypes[i].required) codeArr.push(1);
    else codeArr.push(0);
  }
  let imgCount = 0;
  var count = 0;
  ...
  while (!exhausted) {
    ...
    // const r = 0;
    //r <= desiredCount / totalFaces

    // for (let q = 0; q < partTypes.length; q++) {
    //   if (partTypes[q].name == "hair/hair" && codeArr[q] == 4) {
    //     if (count > 4) {
    //       capped = true;
    //       canContinue = false
    //     } else {
    //       console.log("generated a blondie");
    //       count = count + 1;
    //     }
    // } else if (partTypes[q].name != "hair/hair" && codeArr[q] != 4) {
    //   canContinue = true
    // }
    ...
    for (let i = 0; i < partTypes.length; i++) {
      if (codeArr[i] < partTypes[i].count) {
        ...
        codeArr[i]++;
        for (let j = i - 1; j >= 0; j--) {
          if (partTypes[j].required) codeArr[j] = 1;
          else codeArr[j] = 0;
        }
        break;
      }
